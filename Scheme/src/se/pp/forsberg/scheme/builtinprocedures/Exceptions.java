...
public class Exceptions extends Library {
  public Exceptions() throws SchemeException {
    super();
  }
  ...
  public class _Error extends BuiltInProcedure {
    ...
    @Override public Value apply(Value arguments) throws SchemeException {
      ...
      if (!((Pair)arguments).getCar().isString()) throw new SchemeException("Expected string in " + getName(), (Pair)arguments);
      String msg = (String) ((Pair)arguments).getCar();
      Value irritants = ((Pair)arguments).getCdr();
      throw new SchemeException(msg, irritants);
    }
    @Override
    public Op apply(Op op, Environment env, Value arguments) {
      try {
